...
The `Persistence` project holds the EF Core `AppDbContext` and migrations; the `API` project is the startup project that supplies configuration (connection string) and hosts `Program.cs`. Because `-p`/`-s` are resolved relative to your current directory, **run these commands from the repository root** (`Reactivities\`), not from inside `API\` or `Persistence\`.

If you don't already have the EF Core CLI tool installed:
...
Apply it to create the SQLite database (`reactivities.db`, per `API/appsettings.json`):

```powershell
dotnet ef database update -p Persistence -s API
```

Seeding is handled automatically — `DbInitializer.SeedData` runs on every application startup (see `API/Program.cs`), and skips seeding if the `Activities` table already has rows, so it's safe to leave in place across restarts. You don't need to run it manually.
...
On startup, `Program.cs` calls `context.Database.MigrateAsync()` followed by `DbInitializer.SeedData(context)`, so any pending migrations are applied and the database is seeded automatically each time the API starts — as long as you've created at least one migration as described above.

Alternatively, from the repository root, without hot reload:
